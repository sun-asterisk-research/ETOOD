...
class TaskDecomposition(nn.Module):
    def __init__(self,
        ...
        self.mid_channels = self.in_channels // reduction_rate

        self.channel_attn = nn.Sequential(
            nn.Conv2d(self.in_channels, self.mid_channels, 1),
            nn.Sigmoid()
        )
    ...
    def init_weights(self):
        for m in self.channel_attn.modules():
    ...
    def forward(self, feat, avg_feat):
        b, c, h, w = feat.shape
        weight = self.channel_attn(avg_feat)

        # here we first compute the product between layer attention weight and
        # conv weight, and then compute the convolution between new conv weight
        # and feature map, in order to save memory and FLOPs.
        conv_weight = weight.reshape(
            b, 1, self.mid_channels, 1
        ) * self.reduction_conv.conv.weight.reshape(
            1, self.out_channels, self.mid_channels, self.in_channels // self.mid_channels
        )
        conv_weight = conv_weight.reshape(b, self.out_channels, self.in_channels)
...
class ETOODHead(ATSSHead):
    ...
    def __init__(self,
        ...
        super(ETOODHead, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.cls_decomp = TaskDecomposition(
            self.feat_channels * self.stacked_convs, # in
            self.feat_channels, # out
            self.reduction_rate,
            self.conv_cfg,
            self.norm_cfg
        )
